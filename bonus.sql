...
VALUES
(1, '870101123456', 'Abdirakhman Asset', '+77001234567', 'asset@mail.com', 'active', NOW(), 5000000),
(2, '900202234567', 'Shaykhina Leila', '+77019876543', 'leila@mail.com', 'active', NOW(), 2000000),
(3, '820303345678', 'Shaykhin Erbol', '+77001239876', 'erbol@mail.com', 'blocked', NOW(), 1000000),
(4, '950404456789', 'Abdirakhman Arman', '+77007778888', 'arman@mail.com', 'active', NOW(), 3000000),
(5, '880505567890', 'Abdirakhman Nailya', '+77005556666', 'nailya@mail.com', 'frozen', NOW(), 1000000),
(6, '910606678901', 'Abdirakhman Dariya', '+77004445555', 'dariya@mail.com', 'active', NOW(), 1500000),
(7, '930707789012', 'Dulat Aruzhan', '+77003334444', 'aruzhan@mail.com', 'active', NOW(), 2500000),
(8, '860808890123', 'Nurtaza Daniyal', '+77002223333', 'daniyal@mail.com', 'active', NOW(), 4000000),
(9, '940909901234', 'Nurzhan Yesen', '+77001112222', 'nurzhan@mail.com', 'active', NOW(), 3500000),
(10, '980101012345', 'Nurbek Sembeev', '+77009990000', 'nurbek@mail.com', 'active', NOW(), 6000000);
...
VALUES
(1, 'customers', 1, 'INSERT', NULL, '{"full_name": "Asset Abdirakhman"}', 'system', NOW(), '192.168.0.10'),
...
EXPLAIN ANALYZE SELECT * FROM customers WHERE LOWER(email)='asset@mail.com';
...
-- successful transfer
CALL process_transfer('ACC100001','ACC100002',10000,'kzt','Test');

-- failed transfer
CALL process_transfer('ACC100001','ACC100003',999999999,'kzt','Fail test');

-- salart batch success
CALL process_salary_batch('ACC100001', '[{"iin":"870101123456","amount":100000}]');

-- salary batch  fail
CALL process_salary_batch('ACC100001', '[{"iin":"wrong","amount":50000}]');

--my design decisions:

-- 1/ Hash index on account_number because lookups are equality
-- 2/ Partial index on active accounts to optimize frequent filtering
-- 3/ GIN index on JSONB column for audit_log to improve @> queries
-- 4/ Materialized view used for salary reports because this data is large and being refresh rarely
-- 5/ Advisory locks used in salary batch to prevent batch payments parallels
...
session 2:
-- After session 1 commits and then lock is released
SELECT balance; now it can continue
COMMIT;
*/
