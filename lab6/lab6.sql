...
--output is 4 rows, Tow is not included because he doesnt have deptId

-- 3 2 
SELECT emp_name, dept_name, location
FROM employees
INNER JOIN departments USING (dept_id);
-- when we use "using" with inner join, duplicated columns are not included into the output
...
-- tom brown will have null results in columns dept_dept and dept_name
...
-- null on the left - department without employees
...
INNER JOIN departments d ON e.dept_id = d.dept_id
WHERE d.location = 'Building A';
-- differences only in output, because we actually changing type of the join
-- but no any difference between using ON or WHERE in INNER JOIN
